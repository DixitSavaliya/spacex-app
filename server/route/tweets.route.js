...
tweetsRoutes.get('/twitter-tweets', tweetvalidation.tweets, (req, res) => {
    T.get('/statuses/user_timeline.json?screen_name=SpaceX&count=10', function (err, response) {
        if (err) {
            console.log('err: ', err);
        } else {
...
tweetsRoutes.get('/twitter-tweet', tweetvalidation.tweets, (req, res) => {
    T.get('/statuses/user_timeline.json?screen_name=ElonMusk&count=10', function (err, response) {
        if (err) {
            console.log('err: ', err);
        } else {
            res.status(200).send(response);
